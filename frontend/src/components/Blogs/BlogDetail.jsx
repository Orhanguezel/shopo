...
export default function BlogDetail({ slug }) {
  ...
  if (isError || !data?.blog) return <div className="py-20 text-center">Blog yazisi bulunamadi.</div>;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Yorumunuz onay icin alindi.");
  ...
  return (
    <>
      <Breadcrumb 
        title={blog.title}
        paths={[{ name: "Anasayfa", path: "/" }, { name: "Blog", path: "/blogs" }, { name: blog.title, path: `/blogs/${slug}` }]}
      />
            ...
            <div className="lg:w-2/3">
              <article className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 p-8 md:p-12 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex items-center gap-6 text-sm text-gray-500 mb-8 border-b border-gray-100 pb-8">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="block text-[10px] uppercase font-bold tracking-widest text-gray-400">Yazar</span>
                      <span className="text-gray-900 font-bold">{blog.admin?.name || "Seyfibaba Editoru"}</span>
                    </div>
                  </div>
                  <div className="w-px h-10 bg-gray-100" />
                  <div className="flex flex-col">
                    <span className="block text-[10px] uppercase font-bold tracking-widest text-gray-400">Yayin Tarihi</span>
                    <time className="text-gray-900 font-bold">{format(new Date(blog.created_at), "dd.MM.yyyy")}</time>
                  </div>
                  <div className="w-px h-10 bg-gray-100 hidden md:block" />
                  <div className="hidden md:flex flex-col">
                    <span className="block text-[10px] uppercase font-bold tracking-widest text-gray-400">Kategori</span>
                ...
                <div className="mt-12 pt-8 border-t border-gray-100 flex flex-wrap items-center gap-4">
                  <span className="text-sm font-bold text-gray-900 mr-2 uppercase tracking-widest">Paylas:</span>
                  {[
              ...
              <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 overflow-hidden relative">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-10 flex items-center gap-3 relative">
                  ...
                  Yorum Birak
                  <span className="text-sm font-normal text-gray-500 ml-2">({blog.active_comments?.length || 0} yorum)</span>
                </h3>

                <form onSubmit={handleSubmit} className="space-y-6 relative">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Ad Soyad</label>
                      <input
                        type="text"
                        required
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">E-posta Adresi</label>
                      <input
                        type="email"
                        required
                        className="w-full bg-gray-50 border-0 rounded-2xl p-4 focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-gray-300"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Mesaj</label>
                    <textarea
                      required
                      rows="6"
                      ...
                      placeholder="Yorumunuzu buraya yazin..."
