...
function SlideContent({ item, i }) {
  return (
    <div className="item w-full h-full group relative overflow-hidden">
      <Image
...
function HomeSlider({ images, settings }) {
  ...
  // Show first slide immediately as static HTML before Swiper hydrates
  if (!swiperReady) {
    return (
      <div className="w-full h-full home-slider relative">
        <SlideContent item={images[0]} i={0} />
      </div>
    );
  }
  ...
  return (
    <Slider {...settings} className="w-full h-full home-slider">
      {images.map((item, i) => (
        <SlideContent key={i} item={item} i={i} />
      ))}
    </Slider>
  );
}
