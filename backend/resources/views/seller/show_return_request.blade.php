              ...
              @if ((int) $return->status === 0)
                <form action="{{ route('seller.return-requests.update-status', $return->id) }}" method="POST" class="mb-4">
                  ...
                  <div class="form-group">
                    ...
                    <textarea name="seller_note" class="form-control" rows="4" placeholder="Onay için isteğe bağlı not">{{ old('seller_note', $return->seller_note) }}</textarea>
                  </div>
                  <button type="submit" class="btn btn-primary btn-block">Talebi Onayla</button>
                </form>
                ...
                <form action="{{ route('seller.return-requests.update-status', $return->id) }}" method="POST">
                  ...
                  <div class="form-group">
                    ...
                    <textarea name="rejected_reason" class="form-control" rows="4" required placeholder="Talebin neden reddedildiğini açıklayın">{{ old('rejected_reason', $return->rejected_reason) }}</textarea>
                  </div>
                  <button type="submit" class="btn btn-danger btn-block">Talebi Reddet</button>
                </form>
              @else
