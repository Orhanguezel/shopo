...
/* Company info (telefon, email, adres) */
...
const formatAddress = (a) => {
  ...
  return [line, a.province, cityLine, country].filter(Boolean).join(", ");
};
...
export default function Contact() {
  ...
  // Seçilecek adres (önce default, yoksa ilk)
  const preferredAddress = useMemo(() => {
    const arr = Array.isArray(company?.addresses) ? company.addresses : [];
    return arr.find((x) => x?.isDefault) || arr[0] || null;
  }, [company]);

  const addressText = useMemo(() => formatAddress(preferredAddress), [preferredAddress]);

  // Harita kaynağı (API keysiz fallback → q=adres | varsa lat/lng + EMBED key)
  const mapSrc = useMemo(() => {
    if (!preferredAddress) return "";

    const lat = preferredAddress.lat || preferredAddress.latitude;
    const lng = preferredAddress.lng || preferredAddress.longitude;
    const key = import.meta.env.VITE_GOOGLE_MAPS_EMBED_KEY;

    if (lat && lng && key) {
      return `https://www.google.com/maps/embed/v1/place?key=${key}&q=${lat},${lng}&zoom=14`;
    }

    const line =
      preferredAddress.addressLine ||
      [preferredAddress.street, preferredAddress.houseNumber].filter(Boolean).join(" ");
    const cityLine = [preferredAddress.postalCode, preferredAddress.city].filter(Boolean).join(" ");
    const full = [line, preferredAddress.province, cityLine, preferredAddress.country]
      .filter(Boolean)
      .join(", ");

    return full ? `https://www.google.com/maps?q=${encodeURIComponent(full)}&z=14&output=embed` : "";
  }, [preferredAddress]);
  ...
  const onSubmit = async (e) => {
    ...
    if (!trimmed.name) return toast.error("Please enter your name.");
    if (!trimmed.email) return toast.error("Please enter your email.");
    if (!/^\S+@\S+\.\S+$/.test(trimmed.email)) return toast.error("Please enter a valid email.");
    if (!trimmed.subject) return toast.error("Please enter a subject.");
    if (!trimmed.message) return toast.error("Please enter a message.");
    ...
    try {
      ...
      const msg =
        e2?.data?.message ||
        e2?.error ||
        (typeof e2?.message === "string" ? e2.message : "Could not send message.");
      toast.error(msg);
    }
  };
                ...
                <div className="flex space-x-5">
                  ...
                  <div>
                    ...
                    <p className="text-[15px] text-qblack leading-[30px]">
                      {companyLoading ? "Loading…" : addressText}
                    </p>
                  </div>
                </div>
              ...
              <form className="inputs mt-5" onSubmit={onSubmit}>
                ...
                <div className="mb-4">
                  <InputCom
                    ...
                    inputClasses="h-[50px]"
                    value={mail}
                    inputHandler={(e) => setMail(e.target.value)}
                  />
                </div>
                ...
                {/* company load/error info */}
                <div className="mt-3 text-sm text-qgray">
                  {companyLoading && "Loading company info…"}
                  {companyError && "Company info unavailable."}
                </div>
              </form>
            </div>
          </div>
