  ...
  return (
    <div className="flex">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} w="24" h="24" className={i < full ? "" : "opacity-30"} />
      ))}
    </div>
  );
};
...
export default function SellerCard(props) {
  // Parametre üzerinde default vermek yerine içeride set et
  const {
    title,
    rating = 4.7,
    email = "—",
    coverUrl,
    avatarUrl,
    detailPath = "/saller-page",
    aos = "fade-up",
  } = props;

  // UYARI: ‘assigned but never used’ için güvenli kullanım
  const phone = props.phone ?? "—";
  const addressText = props.addressText ?? "—";

  return (
    <div data-aos={aos} className="item w-full">
      <div
        className="w-full sm:h-[328px] sm:p-[30px] p-5"
        style={{
          background: coverUrl ? `url(${coverUrl}) no-repeat` : undefined,
      ...
      >
        <div className="flex sm:flex-row flex-col-reverse sm:items-center justify-between w-full h-full">
          <div className="flex flex-col justify-between h-full">
            <div>
              <h1 className="text-[30px] font-semibold text-qblack">{title}</h1>
              <div className="flex space-x-2 items-center mb-[30px]">
                <Stars value={rating} />
                <span className="text-[15px] font-bold text-qblack ">({rating})</span>
              </div>
                    ...
                    <div className="flex space-x-2 items-center">
                      ...
                      <span>
                        <svg width="7" height="11" viewBox="0 0 7 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <rect x="1.0918" y="0.636719" width="6.94219" height="1.54271" transform="rotate(45 1.0918 0.636719)" fill="#1D1D1D"/>
                          <rect x="6.00195" y="5.54492" width="6.94219" height="1.54271" transform="rotate(135 6.00195 5.54492)" fill="#1D1D1D"/>
                        </svg>
                      </span>
                    </div>
                  </div>
          ...
          <div>
            <div className="flex sm:justify-center justify-start">
              <div className="w-[170px] h-[170px] rounded-full bg-white mb-[20px] flex justify-center items-center overflow-hidden">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt={title}
                  ...
                  <div className="text-qgray">No Image</div>
                )}
              </div>
            </div>
